...
class connect4():
	...
	def draw_board(self):
		...
		pygame.display.update()
	def check_sequence( start_row, start_col, delta_row, delta_col, player):
			count = 0
			for i in range(4):  # Check up to 4 in a row
				row = start_row + i * delta_row
				col = start_col + i * delta_col
				if 0 <= row < ROW_COUNT and 0 <= col < COLUMN_COUNT and self.board[row][col] == player:
					count += 1
				else:
					break
			return count
